...
def update_data(pos_seqs, seq_nparr, order_label, label_nparr, epoch):
    num_to_add = len(pos_seqs)
    seq_nparr, order_label = remove_old_seq(order_label, seq_nparr, num_to_add)
    print(seq_nparr.shape, pos_seqs.shape)
    seq_nparr = np.concatenate([seq_nparr, pos_seqs])
    order_label = np.concatenate([order_label, np.repeat(epoch, len(pos_seqs))] )
